			...
			if(isset($data['token']) && $data['token'] === $_SESSION['token']){
				...
				if($ex === true){
					...
					if($imgs === 0){
						...
						resizeImageJpeg($_FILES['image']['tmp_name'], '../images/uploads/news/'.$fileName, 847, 564, $_FILES['image']['type'] );
					}
		...
		try {
			if(isset($data['token']) && $data['token'] === $_SESSION['token']){
				...
				if($ex === true){
					...
					if($imgs === 0){
						if($data['imgNow'] !== ""){
							$fileDir = '../images/uploads/news/'.$data['imgNow'];
							$delImg = unlink($fileDir);
							if($delImg === true ){
								list($w, $h) = getimagesize($_FILES['image']['tmp_name']);
								resizeImageJpeg($_FILES['image']['tmp_name'], '../images/uploads/news/'.$fileName, $w, $h, $_FILES['image']['type'] );
							}
						} else {
							list($w, $h) = getimagesize($_FILES['image']['tmp_name']);
							resizeImageJpeg($_FILES['image']['tmp_name'], '../images/uploads/news/'.$fileName, $w, $h, $_FILES['image']['type'] );
	...
	public static function Delete($id, $dbCon, $img ){
		...
		try {
			...
			if($ex === true){
				//deleteNotif();
				if($img !== ""){
					$fileDir = '../images/uploads/news/'.$img;
					$delImg = unlink($fileDir);
					if($delImg === true ){
						backUrl(-1);
					}
